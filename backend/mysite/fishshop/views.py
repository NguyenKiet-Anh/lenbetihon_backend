...
# For encoding images
import base64
# Import libs for image path
from pathlib import Path
import os
# BASE_DIR for getting image
...
def get_path_for_image(path):
    ...
    if os.name == "nt":
        final_path = Path(str(BASE_DIR) + '\\' + relative_path_str)
    elif os.name == "posix":
        final_path = Path(str(BASE_DIR) + '/' + relative_path_str)
    return final_path
...
def getFish(request):     
     ...
     for item in serializers.data:        
        if item["hinh_anh1"]:
            item["hinh_anh1"] = get_path_for_image(get_image_directory(item["hinh_anh1"]))
        ...
        if item["hinh_anh2"]:
            item["hinh_anh2"] = get_path_for_image(get_image_directory(item["hinh_anh2"]))
            with open(item["hinh_anh2"], "rb") as file:
                data = file.read()
                base64_encoded_data = base64.b64encode(data).decode("utf-8")
                item["hinh_anh2"] = base64_encoded_data
        if item["hinh_anh3"]:
            item["hinh_anh3"] = get_path_for_image(get_image_directory(item["hinh_anh3"]))
            with open(item["hinh_anh3"], "rb") as file:
                data = file.read()
                base64_encoded_data = base64.b64encode(data).decode("utf-8")
                item["hinh_anh3"] = base64_encoded_data
        if item["hinh_anh4"]:
            item["hinh_anh4"] = get_path_for_image(get_image_directory(item["hinh_anh4"]))
          ...
          if ma_ca and not check_fish:
               ...
               for item in serializer1.data:      
                    if item["ca_betta_info"]["hinh_anh1"]:
                         item["ca_betta_info"]["hinh_anh1"] = get_path_for_image(get_image_directory(item["ca_betta_info"]["hinh_anh1"]))
...
def selectCart(request):
     ...
     for item in data1:        
          ...
          if item["ca_betta_info"]["hinh_anh2"]:
               item["ca_betta_info"]["hinh_anh2"] = get_path_for_image(get_image_directory(item["ca_betta_info"]["hinh_anh2"]))
          ...
          if item["ca_betta_info"]["hinh_anh3"]:
               item["ca_betta_info"]["hinh_anh3"] = get_path_for_image(get_image_directory(item["ca_betta_info"]["hinh_anh3"]))
          ...
          if item["ca_betta_info"]["hinh_anh4"]:
               item["ca_betta_info"]["hinh_anh4"] = get_path_for_image(get_image_directory(item["ca_betta_info"]["hinh_anh4"]))
